...
public class Weapon : MonoBehaviour
{
    ...
    private void Update()
    {
        if (m_Joystick.isActive)
        {
            ...
            m_Weapon.rotation = Mathf.LerpAngle(m_Weapon.rotation, rotation, m_SpeedRotation * Time.deltaTime);
        }
        Vector2 pointB = m_Player.position + m_Joystick.direction * m_DistanceMove;
        m_Weapon.position = Vector2.Lerp(m_Weapon.position, pointB, m_SpeedMove * Time.deltaTime);
    }
}
